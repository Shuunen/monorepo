 ...
 */ export function sanitize(sentence, willLower = true) {
    const text = removeAccents(sentence).trim() // remove leading and trailing spaces
 ...
 * @returns `"Hello John"`
 */ export function capitalize(string, shouldLower = false) {
 ...
 */ export function createCrc32Table(length = 256) {
    ...
    for(let index = 0; index < length; index += 1){
        ...
        for(let indexB = 0; indexB < 8; indexB += 1)code = code & 0x01 ? 3988292384 ^ code >>> 1 : code >>> 1;
        table[index] = code;
    }
    return table;
}
 ...
 */ export function crc32(text) {
    ...
    for(let index = 0; index < text.length; index += 1){
        var _text_codePointAt;
        /* c8 ignore next */ const code = (_text_codePointAt = text.codePointAt(index)) != null ? _text_codePointAt : 0;
