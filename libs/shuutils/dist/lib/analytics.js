...
export function createAnalytics({ app = 'app-default', onIdentify, onPage, onTrack, willLog = false, willPile = false } = {}) {
    const pile = [];
   ...
   */ function addToPile(item, data) {
        logger == null ? void 0 : logger.debug(`analytics : "${item}" added to pile`, data);
        pile.push(item);
        logger == null ? void 0 : logger.debug(`analytics : pile for app "${app}"`, pile);
   ...
   */ function identify(userId, additionalData) {
        const message = `analytics : identify user "${userId}" on app "${app}"`;
        onIdentify == null ? void 0 : onIdentify(userId, additionalData);
   ...
   */ function track(event, additionalData) {
        ...
        onTrack == null ? void 0 : onTrack(event, additionalData);
        logger == null ? void 0 : logger.debug(message);
