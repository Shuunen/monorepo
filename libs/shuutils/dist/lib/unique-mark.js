 ...
 */ export function getPackageJsonVersion(location = path.join(process.cwd(), 'package.json')) {
    ...
    const { error, value } = Result.unwrap(parseJson(content));
 ...
 */ export async function getTargetFiles(target = (_process_argv_nbThird = process.argv[nbThird]) != null ? _process_argv_nbThird : '') {
    var _RegExp_exec_groups, _RegExp_exec;
    if (target === '') return Result.error('no target specified, aborting.');
    var _RegExp_exec_groups_ext;
    // if ends with something like *.js
    // biome-ignore lint/performance/useTopLevelRegex: should be fixed
    const extension = (_RegExp_exec_groups_ext = (_RegExp_exec = RegExp("\\.(?<ext>[a-z]+)$", "u").exec(target)) == null ? void 0 : (_RegExp_exec_groups = _RegExp_exec.groups) == null ? void 0 : _RegExp_exec_groups.ext) != null ? _RegExp_exec_groups_ext : '';
...
export function injectMarkInFiles({ files = [], isReadOnly = false, mark = 'no-mark', placeholder = 'unique-mark' }) {
    let totalInjections = 0;
    const logs = [];
    const markRegex = new RegExp(mark, 'gu');
    for (const file of files){
        var _updatedContent_match;
        ...
        var _updatedContent_match_length;
        const times = (_updatedContent_match_length = (_updatedContent_match = updatedContent.match(markRegex)) == null ? void 0 : _updatedContent_match.length) != null ? _updatedContent_match_length : 0;
...
var _process_argv_nbThird1;
/**
 * Main function
 * @param target the glob to get the files from, like "public/index.html" or "public/*.js"
 * @returns a Result object
 */ export async function init(target = (_process_argv_nbThird1 = process.argv[nbThird]) != null ? _process_argv_nbThird1 : '') {
    ...
    const result = injectMarkInFiles({
        files: files.value,
        mark
    });
    if (!result.ok) throw new Error(result.error);
    const { logs, totalInjections } = result.value;
    for (const line of logs)logger.info(...line.split(':'));
...
/* c8 ignore start */ if (!isTestEnvironment()) init().catch((error)=>{
    throw error;
});

//# sourceMappingURL=unique-mark.js.map