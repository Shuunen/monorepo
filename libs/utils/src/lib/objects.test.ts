...
describe("objects", () => {
  ...
  it("sort byProperty E with desc when some does not have the property", () => {
  ...
  it("sort byProperty F with asc when first has property but second does not", () => {
    const items = [{ name: "A", value: 10 }, { name: "B" }];
    const sorted = items.toSorted(byProperty("value", "asc"));
    expect(sorted[0]?.name).toBe("A");
  });

  it("sort byProperty G with desc when first has property but second does not", () => {
  ...
  it("sort byProperty H comparator when first has property and second does not", () => {
