...
export function numberSchema(options?: NumberSchemaOptions) {
  ...
  if (min) schema = schema.refine(value => value >= min, { message: `Minimum value is ${min}` })

  const max = options?.max
  if (max) schema = schema.refine(value => value <= max, { message: `Minimum value is ${max}` })
...
export function optionalNumberSchema(options?: NumberSchemaOptions) {
  ...
  if (min !== undefined) schema = schema.refine(value => value === undefined || value >= min, { message: `Minimum value is ${min}` })

  if (max !== undefined) schema = schema.refine(value => value === undefined || value <= max, { message: `Maximum value is ${max}` })
