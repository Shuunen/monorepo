...
function listComponents(startFrom?: string): string[] {
  ...
  if (!startFrom) return components
...
async function processAll(components: string[]) {
  ...
  if (successCount > 0) logger.success(`- ${successCount} components updated`)
  if (errorCount > 0) logger.error(`- ${errorCount} components failed to update`)
}
