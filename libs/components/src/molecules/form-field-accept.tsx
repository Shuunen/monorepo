...
export function FormFieldAccept({ fieldName, fieldSchema, formData, isOptional, logger, readonly = false }: FormFieldBaseProps) {
  ...
  const buttonClasses = cn('flex items-center space-x-2 border rounded-md px-4 py-3', { 'cursor-pointer hover:bg-gray-50': state === 'editable' })
  ...
  return (
    <FormFieldBase {...props}>
      {field => (
        <RadioGroup className="flex" disabled={isDisabled} name={field.name} onValueChange={value => field.onChange(value === 'accepted')} value={field.value === true ? 'accepted' : 'rejected'}>
