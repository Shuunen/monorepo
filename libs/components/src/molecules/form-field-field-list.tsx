...
function AutoFormField({ fieldName, fieldSchema, stepState, logger }: AutoFormFieldProps) {
  const isOptional = fieldSchema instanceof z.ZodOptional;
  const metadata = getFieldMetadata(fieldSchema) ?? {};
  const fieldState = "state" in metadata ? metadata.state : undefined;
  const state = fieldState ?? stepState ?? "editable";
  const props = { fieldName, fieldSchema, isOptional, logger, readonly: state === "readonly" };
          ...
          {items.map((_item, index) => (
            ...
            <div className="flex gap-2" key={index}>
              <div className="flex-1">
                <AutoFormField fieldName={`${fieldName}.${index}`} fieldSchema={itemSchema} logger={logger} />
              </div>
              {index === 0 && (
