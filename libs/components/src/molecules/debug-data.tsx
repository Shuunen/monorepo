...
export function DebugData({ className, data, isFloating = false, isScrollable = true, title, isGhost }: Props) {
  ...
  const classes = cn("relative max-w-full shrink-0 rounded-lg border border-stone-300 bg-stone-100 p-6 shadow-lg dark:bg-stone-800", className, {
  ...
  return (
    <pre className={classes}>
      {title && <strong className="absolute right-0 bottom-0 rounded bg-white px-2 shadow dark:bg-stone-900">{title}</strong>}
