...
export const DateOnly: Story = {
  render: () => <DatetimePicker name="dateOnly" mode="date" />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const dateInput = canvas.getByPlaceholderText("DD/MM/YYYY");
...
export const DateOnlySubmit: Story = {
  render: () => {
    ...
    return (
      <div className="space-y-4">
        <form onSubmit={handleSubmit} className="space-y-4">
          <DatetimePicker name="submitDateNoon" mode="date" onChange={setSelectedDate} />
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(result).toHaveTextContent(/2026-03-15T00:00:00\.\d{3}Z/);
  },
};
...
export const DateOnlyWithNoonSubmit: Story = {
  render: () => {
    const [selectedDate, setSelectedDate] = useState<Date | undefined>();
    const [submittedDate, setSubmittedDate] = useState<string>("");
  ...
  play: async ({ canvasElement }) => {
    ...
    const result = await canvas.findByTestId("submitted-result");
    await expect(result).toBeInTheDocument();
    await expect(result).toHaveTextContent(/2026-03-15T12:00:00\.\d{3}Z/);
  },
};

export const DateAndTime: Story = {
  render: () => <DatetimePicker name="datetime" mode="date-time" />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const dateInput = canvas.getByPlaceholderText("DD/MM/YYYY");
...
export const DateAndTimeWithSubmit: Story = {
  render: () => {
    const [selectedDate, setSelectedDate] = useState<Date | undefined>();
    const [submittedDate, setSubmittedDate] = useState<string>("");
...
export const TimeOnly: Story = {
  render: () => <DatetimePicker name="timeOnly" mode="time" />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const dateInput = canvas.queryByPlaceholderText("DD/MM/YYYY");
    const timeInput = canvas.getByPlaceholderText("--:--");
    const calendarButton = canvas.queryByLabelText("Select date");

    await expect(dateInput).not.toBeInTheDocument();
    await expect(calendarButton).not.toBeInTheDocument();
...
export const TimeOnlyClear: Story = {
  render: () => <DatetimePicker name="clearableTime" mode="time" />,
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const timeInput = canvas.getByPlaceholderText("--:--");
    ...
    const clearButton = canvas.getByLabelText("Clear time");
    await userEvent.click(clearButton);
...
export const ReadonlyTimeOnly: Story = {
  render: () => (
    <DatetimePicker name="readonlyTime" mode="time" readonly defaultValue={new Date(2026, 0, 15, 14, 30)} />
  ),
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const timeInput = canvas.getByPlaceholderText("--:--");
