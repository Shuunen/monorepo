...
export type AutoFormFieldConditionalMetadata = {
  /** The name of another field that this field depends on. Supports field=value and field!=value syntax for specific value checks. Use nested arrays for OR logic: [['a', 'b']] means a OR b. This should be used in favour of `isVisible` when possible for performance reasons. */
  dependsOn?: string | DependsOnCondition[];
  /** More generic way to express condition on whether or not a field is visible. When provided, this function has precedence over `dependsOn`. ⚠️ This should be used as last resort, because the field will be redrawn every time */
