...
export function testIdFromProps(prefix: string, props: NameProp, addValue?: boolean): string {
  if (prefix === '') throw new Error('prefix cannot be empty string when deriving testId from name')
  if (props.name === '') throw new Error('name cannot be empty string when deriving testId from name')
