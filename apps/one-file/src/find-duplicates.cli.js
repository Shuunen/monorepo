...
class CheckDuplicates {
  ...
  args() {
    if (process.argv.length < 4)
      throw new Error(String.raw`this script need a path as argument like : find-duplicates.js "U:\Movies\"`);
  ...
  async check() {
    ...
    for (let aIndex = 0; aIndex < this.nbElements; aIndex += 1)
      for (let bIndex = 0; bIndex < this.nbElements; bIndex += 1) {
        ...
        const sizeA = await stat(path.join(this.target, itemA)).then((/** @type {{ size: number; }} */ data) =>
          Math.round(data.size / sizeGrain),
        );
        // oxlint-disable-next-line no-await-in-loop
        const sizeB = await stat(path.join(this.target, itemB)).then((/** @type {{ size: number; }} */ data) =>
          Math.round(data.size / sizeGrain),
        );
