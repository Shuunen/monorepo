...
// usage : bun ~/Projects/github/monorepo/apps/one-file/src/vivino-compress-export.cli.ts full_wine_list.csv
...
async function init() {
  logger.info(
    "Vivino Compress, will compress your Vivino full_wine_list.csv export file from 170kB to 17kB in seconds",
  );
  await logClear();
  ...
  logger.info(
    "  Done, final file is only",
    Math.round((output.length / input.length) * nbPercentMax),
