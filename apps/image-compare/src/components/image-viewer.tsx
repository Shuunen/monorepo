  ...
  return (
    ...
    >
      ...
      >
        {nbDraggedFiles === 1 && (
          <motion.div animate={{ scale: isDraggingOver ? 1 : 0.9 }} className="flex w-full justify-around" initial={{ scale: 0.9 }} transition={{ duration: 0.2 }}>
            <motion.div animate={{ scale: isDraggingLeft ? 1.2 : 1 }} className={cn('rounded-lg bg-background/80 px-6 py-3 text-lg font-semibold text-foreground', { 'bg-secondary/90': isDraggingLeft })} initial={{ scale: 0.9 }} transition={{ duration: 0.2 }}>
              Change left image
            </motion.div>
            <motion.div animate={{ scale: isDraggingLeft ? 1 : 1.2 }} className={cn('rounded-lg bg-background/80 px-6 py-3 text-lg font-semibold text-foreground', { 'bg-secondary/90': !isDraggingLeft })} initial={{ scale: 0.9 }} transition={{ duration: 0.3 }}>
              Change right image
            </motion.div>
          </motion.div>
        )}
      ...
      <motion.div
        ...
        transition={{ duration: 0.2 }}
      >
        <motion.div className="absolute top-1/2 left-1/2 flex h-12 w-12 -translate-x-1/2 -translate-y-1/2 cursor-grab items-center justify-center rounded-full bg-primary shadow-xl active:cursor-grabbing" test-id="slider-handle" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }}>
      ...
      <motion.div animate={{ opacity: 1, scale: 1 }} className={cn('pointer-events-none absolute right-2 bottom-2 rounded bg-accent/50 px-2 py-1 text-xs text-accent-foreground')} initial={{ opacity: 0, scale: 0.8 }} transition={{ delay: 0.2, duration: 0.3 }}>
