...
function AppItemListEntryComponent({ display, item, showPrice, onSelect, isLoading = false }: Props) {
  ...
  const titleStyle = useMemo(
    () => ({ color: "black", fontSize: 18, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }),
    [],
  );
  ...
  const imgStyle = useMemo(
    () => ({ aspectRatio: 1, objectFit: "contain", padding: "1vw 1vw 0 1vw", width: "100%" }),
    [],
  );
      ...
      {display === "list" && (
        <ListItemButton component="a" disabled={isLoading} href={`/item/details/${item.$id}`} sx={listStyle}>
          <img
            ...
            data-test="item-list-entry-image"
            loading="lazy"
      ...
      {display === "card" && (
        <Card onClick={goToDetails} sx={cardStyle}>
          <CardMedia
            alt={title}
            component="img"
            data-test="item-card-entry-image"
            image={itemToImageUrl(item)}
            loading="lazy"
            sx={imgStyle}
          />
          <ListItemText
