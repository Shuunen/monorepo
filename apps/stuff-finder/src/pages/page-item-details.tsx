...
export function PageItemDetails({ ...properties }: Readonly<Record<string, unknown>>) {
  if (typeof properties.id !== 'string') return <>An id in the url is required, got "{properties.id}"</>
  ...
  const stepsBack = properties.context === 'single' ? 2 : 1 // oxlint-disable-line @typescript-eslint/no-magic-numbers
  return (
