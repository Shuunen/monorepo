...
import {
  addSuggestionsFromAliEx,
  addSuggestionsFromAngbo,
  addSuggestionsFromCampo,
  addSuggestionsFromDeyes,
  addSuggestionsFromWrap,
  cleanSuggestions,
...
it("cleanSuggestions A", () => {
  expect(cleanSuggestions({ keyA: ["a", "b", "c"], keyB: ["a", "b", "c"] })).toEqual({
    keyA: ["a", "b", "c"],
    keyB: ["a", "b", "c"],
  });
});
...
it("cleanSuggestions D", () => {
  expect(cleanSuggestions({ keyA: ["a", "b", "", "0", "c"], keyB: ["a", "b", "c", "a", "b", "c"] })).toEqual({
    keyA: ["a", "b", "c"],
    keyB: ["a", "b", "c"],
  });
});
it("cleanSuggestions E", () => {
  ...
  expect(cleanSuggestions({ name: ["a", -1, "0", null, undefined, "b", "", 0, "c"] })).toEqual({
    name: ["A", "B", "C"],
  });
});
it("cleanSuggestions F", () => {
  // @ts-expect-error typing is not correct
  expect(cleanSuggestions({ details: ["a", "b", "c"], plo: ["0", 0, 12, new Date()], plop: undefined, zob: [""] }))
